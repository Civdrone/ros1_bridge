...
static rclcpp::callback_group::CallbackGroup::SharedPtr get_callback_group(
  ...
  const std::string & topic_name = "")
{
  ...
  typedef std::map<std::string, rclcpp::callback_group::CallbackGroup::SharedPtr> CallbackGroupMap;
...
public:
    ...
    const std::shared_ptr<ROS2Request> request, std::shared_ptr<ROS2Response> response)
  {
    ...
    if (client.call(srv)) {
      ...
      RCLCPP_ERROR(logger, "Failed to get response from ROS 1 service %s", client.getService().c_str());
